...
pub fn get_configuration(base_path: Option<PathBuf>) -> Result<Settings, config::ConfigError> {
    ...
    tracing::debug!(base_path=?base_path, config_dir=?configuration_directory);

    // Detect the running environment
    // Default to `local` if unspecified.
    let environment: Environment = std::env::var("BERRY_ENV")
        .unwrap_or_else(|_| "local".into())
        .try_into()
        .expect("Failed to parse BERRY_ENV");
    ...
    let settings = config::Config::builder()
        ...
        .add_source(
            config::Environment::with_prefix("BERRY")
                .prefix_separator("_")
                .separator("__"),
        )
        .build()?;
